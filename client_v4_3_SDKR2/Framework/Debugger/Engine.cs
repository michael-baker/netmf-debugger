...
namespace Microsoft.SPOT.Debugger
{
    ...
    public class Engine : WireProtocol.IControllerHostLocal, IDisposable
    {
        ...
        internal void NotificationThreadWorker()
        {
            ...
            while (WaitHandle.WaitAny(wh) > 0)
            {
                ...
                while ((read = m_notifyNoise.Available) > 0)
                {
                    ...
                    try
                    {
                        NoiseEventHandler ev = m_eventNoise; if (ev != null) ev(buf, 0, read);
                    }
                    catch
                    {
                    }
                ...
                {
                    object[] arr = (object[])m_notifyQueue[0]; m_notifyQueue.RemoveAt(0);

                    try
                    {
                        CommandEventHandler cev = arr[0] as CommandEventHandler; if (cev != null) cev((WireProtocol.IncomingMessage)arr[1], (bool)arr[2]);
                        MessageEventHandler mev = arr[0] as MessageEventHandler; if (mev != null) mev((WireProtocol.IncomingMessage)arr[1], (string)arr[2]);
                    }
                    catch
                    {
                    }
                ...
                {
                    WireProtocol.IncomingMessage msg = (WireProtocol.IncomingMessage)m_rpcQueue[0]; m_rpcQueue.RemoveAt(0);
        ...
        public bool UpdateSignatureKey(PublicKeyIndex keyIndex, byte[] oldPublicKeySignature, byte[] newPublicKey, byte[] reserveData)
        {
            ...
            if (keyUpdate.m_newPublicKey.Length != newPublicKey.Length) return false;

            if (!keyUpdate.PrepareForSend((uint)keyIndex, oldPublicKeySignature, newPublicKey, reserveData)) return false;
        ...
        private bool ReadMemory(uint address, uint length, byte[] buf, uint offset)
        {
            while (length > 0)
            {
                ...
                if (cmdReply == null || cmdReply.m_data == null) return false;
        ...
        public WireProtocol.Commands.Debugging_Execution_BreakpointDef GetBreakpointStatus()
        {
            ...
            if (reply != null)
            {
                ...
                if (cmdReply != null) return cmdReply.m_lastHit;
            }
        ...
        {
            if (!Capabilities.AppDomains) return null;
        ...
        private bool Deployment_Execute_Full(ArrayList assemblies, MessageHandler mh)
        {
            ...
            if (!Deployment_GetStatus(out entrypoint, out storageStart, out storageLength)) return false;

            if (storageLength == 0) return false;
            ...
            if (deployLength > storageLength) return false;

            if (!EraseMemory(storageStart, deployLength)) return false;
            ...
            foreach (byte[] assembly in assemblies)
            {
                ...
                //
                if (assembly.Length % 4 != 0) return false;

                if (!WriteMemory(storageStart, assembly)) return false;

                storageStart += (uint)assembly.Length;
            }

            if (!WriteMemory(storageStart, closeHeader)) return false;
        ...
        public bool Deployment_Execute(ArrayList assemblies, bool fRebootAfterDeploy, MessageHandler mh)
        {
            ...
            if (!PauseExecution()) return false;

            if (this.Capabilities.IncrementalDeployment)
            {
                if (mh != null) mh("Incrementally deploying assemblies to device");
                fDeployedOK = Deployment_Execute_Incremental(assemblies, mh);
            }
            else
            {
                if (mh != null) mh("Deploying assemblies to device");
                fDeployedOK = Deployment_Execute_Full(assemblies, mh);
            }

            if (!fDeployedOK)
            {
                if (mh != null) mh("Assemblies not successfully deployed to device.");
            }
            else
            {
